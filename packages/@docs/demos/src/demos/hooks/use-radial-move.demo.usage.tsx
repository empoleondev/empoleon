...
const cssCode = `.root {
  ...
  --empty-color: light-dark(var(--empoleon-color-gray-2), var(--empoleon-color-dark-6));
  --filled-color: light-dark(var(--empoleon-color-blue-6), var(--empoleon-color-blue-8));
...
.thumb {
  ...
  &::before {
    ...
    width: 14px;
    height: 14px;
    background-color: light-dark(var(--empoleon-color-white), var(--filled-color));
    border: 2px solid light-dark(var(--filled-color), var(--empoleon-color-white));
    border-radius: 50%;
    left: 50%;
...
function Demo() {
  const [value, setValue] = createSignal(115);
  const { ref } = useRadialMove(setValue);
