...
export const Form = <Form extends UseFormReturnType<any>>(props: FormProps<Form>): JSX.Element => {
  ...
  return (
    <form
      {...others}
      onSubmit={(event) => {
        const formHandler = local.form.onSubmit(
          typeof local.onSubmit === 'function' ? local.onSubmit : () => {}
        );
