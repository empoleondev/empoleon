...
function tests(mode: FormMode) {
  it('initializes form with given initialErrors', () => {
    ...
    expect(hook.result.errors).toStrictEqual({ a: true, b: true });
  });
  ...
  it('allows to set errors object with setErrors handler', () => {
    const hook = renderHook(() => useForm({ mode }));
    hook.result.setErrors({ a: true, b: true });
    expect(hook.result.errors).toStrictEqual({ a: true, b: true });
  });
...
describe('@empoleon/form/errors-controlled', () => {
  tests('controlled');
});

describe('@empoleon/form/errors-uncontrolled', () => {
  tests('uncontrolled');
});
