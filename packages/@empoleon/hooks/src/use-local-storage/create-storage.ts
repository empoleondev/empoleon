...
function serializeJSON<T>(value: T, hookName: string = 'use-local-storage') {
  try {
    ...
    throw new Error(`@empoleon/hooks ${hookName}: Failed to serialize the value`);
  }
}
...
export function createStorage<T>(type: StorageType, hookName: string) {
  const eventName = type === 'localStorage' ? 'empoleon-local-storage' : 'empoleon-session-storage';
