...
describe('@empoleon/core/Text', () => {
  tests.itSupportsSystemProps<TextProps, TextStylesNames>({
    ...
    displayName: '@empoleon/core/Text',
  ...
  it('sets data-truncate attribute based on truncate prop', () => {
    ...
    rerender(() => <Text />);
    expect(container.querySelector('.empoleon-Text-root')).not.toHaveAttribute('data-truncate');
  });

  it('sets data-variant attribute based on variant prop', () => {
    const { container, rerender } = render(() => <Text variant="text" />);
    expect(container.querySelector('.empoleon-Text-root')).toHaveAttribute('data-variant', 'text');
  ...
  it('sets data-inline attribute based on inline prop', () => {
    ...
    expect(container.querySelector('.empoleon-Text-root')).toHaveAttribute('data-inline');

    rerender(() => <Text />);
    expect(container.querySelector('.empoleon-Text-root')).not.toHaveAttribute('data-inline');
  });

  it('sets data-inherit attribute based on inherit prop', () => {
    const { container, rerender } = render(() => <Text inherit />);
    expect(container.querySelector('.empoleon-Text-root')).toHaveAttribute('data-inherit');

    rerender(() => <Text inherit={false} />);
    expect(container.querySelector('.empoleon-Text-root')).not.toHaveAttribute('data-inherit');
  });

  it('sets data-line-clamp attribute based on lineClamp prop', () => {
    const { container, rerender } = render(() => <Text lineClamp={3} />);
    expect(container.querySelector('.empoleon-Text-root')).toHaveAttribute('data-line-clamp');

    rerender(() => <Text />);
    expect(container.querySelector('.empoleon-Text-root')).not.toHaveAttribute('data-line-clamp');
  });

  it('changes root element to span if span prop is set', () => {
    const { container } = render(() => <Text span />);
    expect(container.querySelector('.empoleon-Text-root')!.nodeName.toLowerCase()).toBe('span');
  });
});
