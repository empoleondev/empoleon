...
describe('@empoleon/core/Table', () => {
  tests.itSupportsSystemProps<TableProps, TableStylesNames>({
    ...
    displayName: '@empoleon/core/Table',
  ...
  it('renders table rows based on data prop', () => {
    ...
    expect(container.querySelector('thead th')?.textContent).toBe('test-thead');
    expect(container.querySelector('tfoot th')?.textContent).toBe('test-tfoot');
    expect(container.querySelector('caption')?.textContent).toBe('test-caption');
  });
