...
describe('@empoleon/core/Button', () => {
  ...
  tests.itSupportsSystemProps<ButtonProps, ButtonStylesNames>({
    ...
    displayName: '@empoleon/core/Button',
  ...
  it('renders given left section', () => {
    const { container } = render(() => <Button leftSection="test-left-section" />);
    const element = container.querySelector('.empoleon-Button-section[data-position="left"]');
    expect(element?.textContent).toBe('test-left-section');
  });

  it('renders given right section', () => {
    const { container } = render(() => <Button rightSection="test-right-section" />);
    const element = container.querySelector('.empoleon-Button-section[data-position="right"]');
    expect(element?.textContent).toBe('test-right-section');
  });
