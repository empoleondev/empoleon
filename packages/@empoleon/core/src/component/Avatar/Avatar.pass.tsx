...
describe('@empoleon/core/Avatar', () => {
  ...
  tests.itSupportsSystemProps<AvatarProps, AvatarStylesNames>({
    ...
    displayName: '@empoleon/core/Avatar',
  ...
  it('sets data-within-group attribute when Avatar is rendered inside Avatar.Group', () => {
    ...
    rerender(() => <Avatar />);
    expect(container.querySelector('.empoleon-Avatar-root')).not.toHaveAttribute(
      'data-within-group'
    );
  });
  ...
  it('renders placeholder if src was not set', () => {
    ...
    expect(container.querySelector('.empoleon-Avatar-placeholder')).toBeInTheDocument();

    rerender(() => <Avatar src="image.png" />);
    expect(container.querySelector('.empoleon-Avatar-placeholder')).not.toBeInTheDocument();
