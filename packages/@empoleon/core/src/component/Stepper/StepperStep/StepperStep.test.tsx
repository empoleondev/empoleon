...
describe('@empoleon/core/StepperStep', () => {
  tests.itSupportsSystemProps<StepperStepProps, StepperStepStylesNames>({
    ...
    displayName: '@empoleon/core/StepperStep',
    stylesApiSelectors: [
      'step',
      'stepBody',
      'stepDescription',
    ...
    selector: '.empoleon-Stepper-step',
  });

  it('renders icon when withIcon is true', () => {
    const { rerender, container } = render(() => <TestContainer withIcon={false} />);
    expect(container.querySelectorAll('.empoleon-Stepper-stepIcon')).toHaveLength(0);

    rerender(() => <TestContainer withIcon />);
    expect(container.querySelectorAll('.empoleon-Stepper-stepIcon')).toHaveLength(1);
  });
