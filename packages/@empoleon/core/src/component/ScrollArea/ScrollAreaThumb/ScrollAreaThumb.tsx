...
export function Thumb(props: ThumbProps) {
  const [local, others] = splitProps(props, ['style', 'onPointerDown', 'onPointerUp', 'ref']);

  const scrollAreaContext = useScrollAreaContext();
  const scrollbarContext = useScrollbarContext();
  const { onThumbPositionChange } = scrollbarContext;
  const composedRef = useMergedRef(local.ref, (el: HTMLDivElement) =>
    scrollbarContext.onThumbChange(el)
  );
  ...
  return (
    <div
      ...
      onPointerDown={composeEventHandlers(
        local.onPointerDown as (
          e: PointerEvent & { currentTarget: HTMLDivElement; target: Element }
        ) => void,
        (event) => {
...
export function ScrollAreaThumb(props: ScrollAreaThumbProps) {
  const [local, thumbProps] = splitProps(props, ['forceMount', 'ref']);
  const scrollbarContext = useScrollbarContext();
