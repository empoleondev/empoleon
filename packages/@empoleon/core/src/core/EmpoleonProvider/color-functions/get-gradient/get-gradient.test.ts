...
describe('@empoleon/core/get-gradient', () => {
  it('returns default gradient from theme if gradient is not provided', () => {
    expect(getGradient(undefined, DEFAULT_THEME)).toStrictEqual(
      'linear-gradient(45deg, var(--empoleon-color-blue-filled) 0%, var(--empoleon-color-cyan-filled) 100%)'
    );
  });
  ...
  it('merges given gradient with theme default gradient', () => {
    expect(
      ...
      'linear-gradient(90deg, var(--empoleon-color-red-filled) 0%, var(--empoleon-color-blue-filled) 100%)'
    );
  });
  ...
  it('get colors from theme by index', () => {
    expect(
      ...
      'linear-gradient(90deg, var(--empoleon-color-red-3) 0%, var(--empoleon-color-blue-8) 100%)'
    );
  });
