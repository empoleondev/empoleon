...
const THEME_WITH_CLASSNAMES: EmpoleonTheme = {
  ...DEFAULT_THEME,
  components: {
    TestComponentObject: {
      classNames: {
...
describe('@empoleon/core/get-class-name', () => {
  ...
  it('resolves static selectors correctly', () => {
    ...
    expect(output.includes('empoleon-Test-root')).toBe(true);
    expect(output.includes('empoleon-AnotherTest-root')).toBe(true);
  });
  ...
  it('does not add static classes if withStaticClasses is false', () => {
    ...
    expect(output.includes('empoleon-Test-root')).toBe(false);
    expect(output.includes('empoleon-AnotherTest-root')).toBe(false);
