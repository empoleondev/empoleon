...
import { getEmpoleonPackagesList } from '../packages/get-packages-list';
import { publishPackage } from '../publish/publish-package';
import { getPath } from '../utils/get-path';
import { createLogger } from '../utils/signale';
import { openGithubRelease } from './open-github-release';
import { setEmpoleonPackagesVersion } from './set-empoleon-packages-version';
...
async function release() {
  ...
  await setEmpoleonPackagesVersion(incrementedVersion);
  ...
  const empoleonPackages = await getEmpoleonPackagesList();

  await Promise.all(
    empoleonPackages.map((p) =>
